...
public class DroolsTest {
    ...
    public static void main(String[] args) throws ClassNotFoundException, SQLException {
        ...
        } else if (!rule_folder.equals("")) {
            kbase = kContainer.newKieBase(rule_folder, kconfig);
        } else {
        ...
        ResultSet dexpQuery = dexpSt.executeQuery(
                "SELECT"
                        ...
                        + " FROM drug_exposure dexp"
                            + " INNER JOIN vocabulary.drug_strength dstr ON dexp.drug_concept_id = dstr.drug_concept_id"
                        + " INNER JOIN vocabulary.concept cd ON dexp.drug_concept_id = cd.concept_id"
                        + " INNER JOIN vocabulary.concept ci ON dstr.ingredient_concept_id = ci.concept_id"
                        + " LEFT JOIN scratch_erc53fhir.sig_mapping sm ON dexp.sig = sm.sig"
