...
with open("data.csv", "rb") as infile, open("output.sql", "wb") as outfile:
  ...
  for row in reader:
    if row[0] == "PER":
      out_string = ("INSERT INTO person (person_id, gender_concept_id, year_of_birth, month_of_birth, day_of_birth, race_concept_id, ethnicity_concept_id, location_id) values (" + row[1] + ", " + row[14] + ", " + row[2] + ", " + row[10] +", " + row[11] + ", 8527, 38003564, 321);\n")
      outfile.write(out_string.encode())
